...
| ident (n : α) (name : QualifiedIdent) (args : Array (TypeExprF α))
...
| fvar (n : α) (fvar : FreeVarIndex) (args : Array (TypeExprF α))
  /-- A function type. -/
| arrow (n : α) (arg : TypeExprF α) (res : TypeExprF α)
deriving Inhabited, Repr
...
protected def hasUnboundVar (bindingCount : Nat := 0) : TypeExprF α → Bool
...
protected def isGround (tp : TypeExprF α) := !tp.hasUnboundVar
...
| decimal (v : Decimal)
| strlit (i : String)
| option (l : Option (ArgF α))
| seq (l : Array (ArgF α))
| commaSepList (l : Array (ArgF α))
deriving Inhabited, Repr
...
theorem sizeOf_lt_of_op_arg {e : ArgF α} {op : OperationF α} (p : e ∈ op.args) : sizeOf e < sizeOf op := by
  cases op with
  | mk ann name args =>
...
-/
structure HNF (e : ExprF α) where
  fn : ExprF α
  args : SizeBounded (Array (ArgF α)) e 1

protected def hnf (e0 : ExprF α) : HNF e0 :=
...
def asType! [Inhabited α] [Repr α] : ArgF α → TypeExprF α
| .type a => a
...
def ofType : TypeExprF α → PreType
| .ident _ name args => .ident name (args.attach.map fun ⟨a, _⟩ => .ofType a)
| .bvar _ idx => .bvar idx
| .fvar _ idx args => .fvar idx (args.attach.map fun ⟨a, _⟩ => .ofType a)
| .arrow _ a r => .arrow (.ofType a) (.ofType r)
...
def nameIndex : BindingSpec argDecls → DebruijnIndex argDecls.size
| .value v => v.nameIndex
| .type v => v.nameIndex

def varName (b : BindingSpec argDecls) (args : Vector Arg argDecls.size) : String :=
...
-/
partial def OperationF.foldBindingSpecs
    (m : DialectMap)
    (f : β → α → ∀{argDecls : ArgDecls}, BindingSpec argDecls → Vector (ArgF α) argDecls.size → β)
    (init : β)
    (op : OperationF α)
...
partial def resolveBindingIndices { argDecls : ArgDecls } (m : DialectMap) (src : SourceLoc) (b : BindingSpec argDecls) (args : Vector Arg argDecls.size) : GlobalKind :=
  match b with
  | .value b =>
    ...
    | .type tp =>
      ...
      | none =>
        .expr tp
      | some idx =>
        let f (a : Array _) (l : SourceLoc) {argDecls : ArgDecls} (b : BindingSpec argDecls) args :=
                let type :=
  ...
  | .type b =>
    let params : Array String :=
        ...
        | none => #[]
        | some idx =>
          let addBinding (a : Array String) (_ : SourceLoc) {argDecls : _} (b : BindingSpec argDecls) (args : Vector Arg argDecls.size) :=
              a.push (varNameByIndex args b.nameIndex)
...
def addCommand (dialects : DialectMap) (init : GlobalContext) (op : Operation) : GlobalContext :=
  ...
  where addBinding gctx l _ b args :=
